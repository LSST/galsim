...
if [[ $OSTYPE == darwin* ]]; then
	...
	LIBPYTHON_DYLIB=$(python -c "from __future__ import print_function; import sysconfig, os.path; print(os.path.join(*sysconfig.get_config_vars('LIBDIR', 'LDLIBRARY')))")
	...
	if [[ $BROKEN_LIBPYTHON_DYLIB = 1 ]]; then
		...
		# at runtime
		DYLD_FALLBACK_LIBRARY_PATH=$(python -c "from __future__ import print_function; import sysconfig; print(sysconfig.get_config_var('LIBDIR'))")
